...
<html lang="en">
	<head>
		<title>Pharmacy | View All Pharmacy</title>
...
<div class="row">
<div class="col-md-12">
<h5 class="over-title margin-bottom-15"><?php echo $disName ?> <span class="text-bold">View All Details</span></h5>
...
// $sqltp=mysqli_query($con,"SELECT * FROM tbl_pharmacy where pharmacy_id = '$dispensary_Id'");
$sqltp=mysqli_query($con,"select tbl_userlogins.* , tbl_pharmacy.* from tbl_userlogins
INNER JOIN tbl_pharmacy
on tbl_userlogins.user_id = tbl_pharmacy.user_id
WHERE tbl_pharmacy.user_id = '$disId' ");
...
// // "SELECT * FROM tbl_pharmacy WHERE pharmacy_id='$disId' ");
//     $ret=mysqli_query($con,"select tbl_userlogins.* , tbl_pharmacy.* from tbl_userlogins
// 	INNER JOIN tbl_pharmacy
// 	on tbl_userlogins.user_id = tbl_pharmacy.user_id
// 	WHERE tbl_pharmacy.user_id = '$disId' "
...
while($row=mysqli_fetch_array($sqltp))
{
	...
	<a href="view-dispensary_details.php?viewid=<?php echo $row['pharmacy_id'];?>"><i class="fa fa-eye"></i></a>
